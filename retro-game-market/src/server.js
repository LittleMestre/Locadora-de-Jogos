...
// 2. GET - Buscar jogo por código
app.get('/api/games/:id', (req, res) => {
  const id = req.params.id;
  db.query('SELECT * FROM games WHERE codigo = ?', [id], (err, results) => {
    if (err) {
...
app.post('/api/games', (req, res) => {
  const { nome, genero, classificacao, preco, quantidade, imagem, descricao } = req.body;
  ...
  const game = { nome, genero, classificacao, preco, quantidade, imagem, descricao };
...
app.put('/api/games/:id', (req, res) => {
  ...
  db.query('UPDATE games SET ? WHERE codigo = ?', [game, id], (err, result) => {
    if (err) {
...
app.delete('/api/games/:id', (req, res) => {
  ...
  db.query('DELETE FROM games WHERE codigo = ?', [id], (err, result) => {
    if (err) {
    ...
    res.json({ message: `Jogo com código ${id} excluído com sucesso` });
  });
});
